...
# Make predictions
y_pred = ebm.predict(X_test)
print(f<span>"Accuracy: {accuracy_score(y_test, y_pred)}"</span>)
```
...
```python
# Interpret the model
ebm_global = ebm.explain_global(name=<span>'EBM'</span>)

# Extract feature importances
feature_names = ebm_global.data()[<span>'names'</span>]
importances = ebm_global.data()[<span>'scores'</span>]
